...
public class ServiceImplGenerator extends AbstractCodeGenerator {
    ...
    public boolean supports(GeneratorType type) {
        return type == GeneratorType.SERVICE_IMPL;
    }
    ...
    private String buildImports(GeneratorContext context) {
        ...
        // Service 接口导入
        imports.append(String.format("import %s.service.%sService;\n", basePackage, className));
    ...
    private String buildImplementationClass(GeneratorContext context) {
        String className = toClassName(context.getTableName());
        String implName = className + "ServiceImpl";
        ...
        declaration.append(" */\n");
        declaration.append("@Service\n");
        declaration.append(String.format("public class %s extends BaseService<%s, Long> implements %sService {\n", implName, className, className));

        // DAO 字段
        declaration.append(String.format("    private final %sDao %sDao;\n\n", className, entityName));

        // 构造器
        declaration.append("    @Autowired\n");
        declaration.append(String.format("    public %s(%sDao %sDao) {\n", implName, className, entityName));
        declaration.append(String.format("        super(%sDao);\n", entityName));
        declaration.append(String.format("        this.%sDao = %sDao;\n", entityName, entityName));
        declaration.append("    }\n");
