...
@Slf4j
@Component
public class MysqlMapperXmlGenerator extends AbstractCodeGenerator {
    ...
    public boolean supports(GeneratorType type) {
        return type == GeneratorType.MAPPER_XML && isMySqlDatabase();
    }
    ...
    private boolean isMySqlDatabase() {
        ...
        try {
            if (environment == null) {
                log.debug("Environment is null, returning false for MySQL check");
                return false;
            }
            var driverClassName = environment.getProperty("spring.datasource.driver-class-name", "");
            log.debug("Checking MySQL: driverClassName={}", driverClassName);
            boolean isMySql = "com.mysql.cj.jdbc.Driver".equals(driverClassName) || "com.mysql.jdbc.Driver".equals(driverClassName);
            log.debug("MySQL check result: {}", isMySql);
            return isMySql;
        } catch (Exception e) {
            log.error("Error checking MySQL database type", e);
            return false;
        }
    }
    ...
    private boolean shouldIncludeFieldInInsert(String columnName) {
        return switch (columnName) {
            case "id",
                 "updater", "updater_id", "updated_time",
